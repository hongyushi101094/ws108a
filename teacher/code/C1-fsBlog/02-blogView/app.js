const fs = require('fs')//fs讀取文檔
const path = require('path')//path路徑
const Koa = require('koa')//伺服器
const KoaRouter = require('koa-router')//路由器
const koaLogger = require('koa-logger')//用法
const MarkdownIt = require('markdown-it')//可以轉換成html的一個套件
const mdit = new MarkdownIt({ // 創建 markdown 的轉換物件！
  html: true,//開啟可以使用html的一個開關
  linkify: true,//開啟可以使用連結的一個開關
  typographer: true
})

const app = module.exports = new Koa()
const router = new KoaRouter()//創建koaRouter 的轉換物件

router
.get('/', async (ctx) => {
  ctx.redirect('/blog/Home')
})//當只有http://localhost:3000/ 的時候自動轉到http://localhost:3000/blog/Home
